...
namespace CcrSpaces.Channels
{
    public static class CcrSpaceExtensions
    {
        ...
        public static PortSet<TInput, CcrsRequest<TInput, TOutput>, CcrsRequestOfUnknownType> CreateChannel<TInput, TOutput>(this ICcrSpace space, CcrsRequestResponseChannelConfig<TInput, TOutput> config)
        {
            config.TaskQueue = config.TaskQueue ?? space.DefaultTaskQueue;
            return CcrsChannelFactory.Instance.CreateChannel(config);
        }
